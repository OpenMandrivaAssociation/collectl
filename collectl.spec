...
mkdir -p  ${RPM_BUILD_ROOT}/var/log/%{name} ${RPM_BUILD_ROOT}%{_sbindir} ${RPM_BUILD_ROOT}%{_docdir}/%{name}-%{version} ${RPM_BUILD_ROOT}%{_sysconfdir}/init.d/ ${RPM_BUILD_ROOT}%{_mandir}/man1/
...
install -m 444  RELEASE-%{name} FAQ-%{name}.html ${RPM_BUILD_ROOT}%{_docdir}/%{name}-%{version}
